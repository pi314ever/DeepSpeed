...
class FP8_Optimizer(ZeROOptimizer):
    ...
    def _setup_for_real_optimizer(self):
        ...
        if self.master_weights_dtype == torch.float16:
            self.calculate_fp16_scale = self.optimizer.param_groups[0]['calculate_fp16_scale']
            self.cast_to_fp32 = self.optimizer.param_groups[0]['cast_to_fp32']
            self.cast_to_fp16 = self.optimizer.param_groups[0]['cast_to_fp16']
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            if self.master_weights_dtype == torch.float16:
                assert len(trainable_parameters) == 1, \
                    f"should be one param in each group when we use fp16 master weights with per tensor scaling"
    ...
    def _setup_for_bf16_params(self, real_dp_world_size, i, param_group, partition_id, trainable_parameters):
        ...
        if self.master_weights_dtype == torch.float16:
            partition = bf16_dp_partitions[partition_id].clone().detach()
            self.calculate_fp16_scale(partition, group=self.real_dp_process_group[i])
            partition.data = self.cast_to_fp16(partition).data
            self.fp32_groups_flat_partition.append(partition)
        else:
        ...
        for dtype in dtypes:
            ...
            self.fp8_param_groups_dict[dtype]["param_groups_flat"].append(
                self._flatten_dense_tensors_aligned(self.fp8_param_groups_dict[dtype]["param_groups"][i],
                                                    self.nccl_start_alignment_factor * real_dp_world_size))
            # Make params point to flat tensor storage
            ...
            self.fp8_param_groups_dict[dtype]["param_partitioned_groups"].append(dp_partitions)
            if first_dtype:
                # create fp32 params partition
                if self.master_weights_dtype == torch.float16:
                    partition = dp_partitions[partition_id].clone().detach()
                    self.calculate_fp16_scale(partition, group=self.real_dp_process_group[i])
                    partition.data = self.cast_to_fp16(partition).data
                    self.fp32_groups_flat_partition.append(partition)
                else:
                    self.fp32_groups_flat_partition.append(dp_partitions[partition_id].clone().to(
                        self.master_weights_dtype).detach())
                self.fp32_groups_flat_partition[i].requires_grad = True
                ...
                # convert bf16 params to fp8 params
                for p in trainable_parameters:
                    p.all_data[dtype] = p.cast_to(dtype, p.data)
                    p.data = p.all_data[dtype]
                # update flat tensor to flat fp8 params
                self.fp8_param_groups_dict[dtype]["param_groups_flat"][i] = self._flatten_dense_tensors_aligned(
                    self.fp8_param_groups_dict[dtype]["param_groups"][i],
                    self.nccl_start_alignment_factor * real_dp_world_size)
                # update fp8 params point to flat tensor storage
                self._update_storage_to_flattened_tensor(
                    tensor_list=self.fp8_param_groups_dict[dtype]["param_groups"][i],
                    flat_tensor=self.fp8_param_groups_dict[dtype]["param_groups_flat"][i])
                # divide flat weights into equal sized partitions
                partition_size = self.fp8_param_groups_dict[dtype]["param_groups_flat"][i].numel(
                ) // real_dp_world_size
                dp_partitions = [
                    self.fp8_param_groups_dict[dtype]["param_groups_flat"][i].narrow(
                        0, dp_index * partition_size, partition_size) for dp_index in range(real_dp_world_size)
                ]
                self.fp8_param_groups_dict[dtype]["param_partitioned_groups"][i] = dp_partitions
                first_dtype = False
    ...
    def _update_storage_to_flattened_tensor(self, tensor_list, flat_tensor):
        ...
        for p, q in zip(tensor_list, updated_params):
            p.data = q.data
            if hasattr(p, 'all_data') and p.dtype in p.all_data:
    ...
    def step(self, closure=None):
        ...
        for param_partition, grad_partition in zip(self.fp32_groups_flat_partition,
                                                   self.fp32_groups_gradient_flat_partition):
            if self.master_weights_dtype is torch.float16:
                param_partition.prev = param_partition.data
                param_partition.data = self.cast_to_fp32(param_partition)
        ...
        if self.master_weights_dtype is torch.float16:
            for i, param_partition in enumerate(self.fp32_groups_flat_partition):
                self.calculate_fp16_scale(param_partition, group=self.real_dp_process_group[i])
                param_partition.prev.copy_(self.cast_to_fp16(param_partition))
                param_partition.data = param_partition.prev
            ...
            if partition_group is None:
                ...
                for p in params:
                    if p._hp_mapping is not None:
                        for dtype in dtypes:
                            fp8_partition_data_temp = p.cast_to(dtype, p._hp_mapping.hp_fragment.data)
